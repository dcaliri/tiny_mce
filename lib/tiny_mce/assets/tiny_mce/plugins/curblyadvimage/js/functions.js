function cancel_image(){
  jQuery('.attachment_list').removeClass("selected")
}

function delete_attachment(element){
  jQuery('#dynamic_images_list').html("Uploading...<br /><img src='/images/tiny_mce/spinner.gif'>");
  jQuery('.attachment_list').removeClass("selected")
...
function upload_callback(){
    //jQuery('#dynamic_images_list').html('');
    jQuery.ajax({type: "GET", url: ts_upload_image_path(), dataType: "script"});
}
...
function ts_upload_image_path() {
  path_prefix = parent.opener.location.pathname.split("/")[3];
  to_path = "/admin/organizers/" + path_prefix + "/media_contents/";
  return to_path;
}
